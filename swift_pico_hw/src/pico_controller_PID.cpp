...
#include "rc_msgs/msg/rc_message.hpp"
...
class Swift_Pico : public rclcpp::Node
{
    public:
        Swift_Pico() : Node("pico_controller") //initializing ros node with name pico_controller
        {
            ...
            command_pub = this->create_publisher<rc_msgs::msg::RCMessage>("/drone_command", 10);
            ...
            //Create a timer to run the pid function periodically, refer ROS 2 tutorials on how to create a publisher subscriber(C++)
    ...
    private:
        ...
        void pid()
        {
            auto cmd = rc_msgs::msg::RCMessage();
            ...
            #	5. Don't run the pid continuously. Run the pid only at the a sample time. self.sampletime defined above is for this purpose. THIS IS VERY IMPORTANT.
